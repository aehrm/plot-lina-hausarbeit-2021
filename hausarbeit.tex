...
%
Nach Trilcke et~al. sei zentrale Schwachstelle dieser Figurennetzwerke, dass diese als \emph{statischen} Modelle die zeitliche Dimension von Plot nicht abgebilden können.\autocite[Vgl.][176]{trickle_netzwerkdynamik_2017}

Diese Arbeit folgt daher deren generellem Forschungsdesiderat: Die Netzwerkanalyse im Rahmen von Dramen sei in Richtung einer quantitativen Plot-Analyse weiterzuentwickeln.
Aktuelle Entwicklungen in der Netzwerkanalyse versuchen daher, \emph{dynamische} Netzwerke zu entwickeln, um sie für die Plot-Analyse zu gebrauchen.
...
\end{enumerate*}
Letzter Punkt soll insbesondere beitragen, aus dieser Plot-Strukturierung dann Kompositionsprinzipien von Plot in digitalen Korpora messbar zu machen.
...
Die Komposition der Handlugnssequenze ist meist durchaus komplexer.
...
Gerade im Hinblick auf Fragen betreffend der hierarchischen Abstufung (\textquote*{Haupthandlung} vs. \textquote*{Nebenhandlung}) ist es vorteilhaft, wenn eine solche Segmentierung thematisch ähnliche Handlungen in Sequenzen zusammenfass.
...
Es sei hier außerdem angemerkt, dass die Umkehrung obiger Annahme nicht zutrifft: Eine Handlung der Gesamthandlung des Dramas muss im Allgemeinen nicht immer in einer Situation dargestellt werden.
...
Stattdessen greifen wir auf manuell annotierte digitale Editionen dramatischer Texte zurück.
...
\begin{enumerate*}
    \item Jede Szene entspricht genau einer Situation.
        Wir gehen also davon aus, dass währen der Szene keine Veränderung der präsenten Figuren erfolgt.
...
Es ist zwar klar, dass diese Annahmen im Allgemeinen nicht zutreffen, wir treffen diese Entscheidungen aber im Hinblick auf eine quantitative Auswertung großer digitaler TEI-genormter Korpora wie \emph{GerDraCor}.
Ein potentielles Modell muss daher mit diesen Imperfektionen der Eingangsdaten umgehen.


\subsection{Graphen}

Wir folgen den Ausführungen von Krumke und Noltemeier für eine formal präzise Definition des Graphens.
...
\begin{enumerate*}
    ...
    \item Die Funktionen $\alpha$ und $\omega$ bilden von $R$ nach $V$ ab.
        Für $r\in R$ ist $\alpha(r)$ die \emph{Anfangsecke} von $r$, und $\omega(r)$ die \emph{Endecke} von $r$.
...
Unmittelbar aus dieser Darstellungsweise folgt der Begriff des \emph{Wegs}: ein Weg $P$ ist eine endliche Folge $(v_0, r_1, v_1, \dots, r_k, v_k)$, $k\geq 0$, sodass $v_0, \dots, v_k$ Ecken sind, $r_1, \dots, r_k$ Pfeile sind, sowie $\alpha(r_i) = v_{i-1}$, $\omega(r_i)=v_i$ für alle $i=1,\dots, k$.
...
Ein \emph{gerichteter azyklischer Graph} ist ein Graph, welcher kein Kreis enthält.
Also kein Weg $P$ der Länge $\geq 1$ existiert, sodass erste und letzte Ecke der Spur identisch sind.
...
Jede Zeile dieser Matrix repräsentiert hierbei eine Figur, jede Spalte eine zur Situation und dessen korrespondierende Konfiguration.
...
\begin{enumerate*}
    ...
    \item Figurennetzwerke werden durch eine zeitliche Dimension angereichert.
...
Die dlina-Arbeitsgruppe führt die von ihnen benannte \emph{segment-change rate} auf Basis der Konfigurationsmatrix ein, um Handlungsentwicklungen zu erfassen.\autocites{trickle_netzwerkdynamik_2017}{fischer_network_2017}{trilcke_small_2020}
Neben anderen eingeführten Maßen bearbeiten sie die \emph{segment-change rate} dabei in ihren Vorträgen und veröffentlichen Arbeiten besonders intensiv.

Seien $s$ und $s'$ unmittelbar konsekutive Situationen.
Seien hierzu $K_s$ bzw. $K_{s'}$ die Konfigurationen (als Mengen von Figuren) zu $s$ bzw. $s'$.
...
Diese Arbeit der dlina-Arbeitsgruppe wird hier deshalb so intensiv erläutert, da sie der Konfigurationsanalyse zugeordnet werden kann – wie die hier vorliegende Arbeit.
...
In dieser Hinsicht sieht die Arbeitsgruppe zum Anderen mehr Potential in der \textquote{Berechnung netzwerkanalytischer Maße \textins{von dynamischen Netzwerken} und deren statistische Weiterverarbeitung}\autocites[178]{trickle_netzwerkdynamik_2017}[Vgl. auch][438]{fischer_network_2017}
...
Inwiefern \emph{segment-change rates} ein Maß dynamischer Netzwerke darstellt, ist dem Autor der hier vorliegenden Arbeit nicht ersichtlich, sind ja die beschriebenen Konfigurationsmatrizen weder Netzwerke noch dynamisch.

Näher an der Idee von Maßen dynamischer Netzwerke ist die von den Autoren zitierte Arbeit von Prado et~al. 
Diese greift von Taylor et~al. entwickelte Zentralitätsmaße für \textquote{\eng{temporal networks}} auf, verwendet diese für literarische Fragestellungen, und wendet diese auf zeitlich variablen Figurennetzwerken an.\autocites{prado_temporal_2016}{taylor_eigenvector-based_2015}
...
Oberflächlich gesprochen bilden diese zu einem dramatischen Werk das Netzwerk der Situationen und dessen syntagmatischen Relationen ab.
Die Präsentation geht hierbei rückwärts vor: Erst liefern wir eine mathematisch eindeutige und präzise Definition.
...
Um eine präzise Angabe des Graphens möglich zu machen, führen wir einige mathematische Objekte ein, welche sich aus der Struktur des Dramas ergeben.
...
\begin{SidewaysFigure}
    ...
    Bis auf Angelo, Conti und den Kammerdiener ist dem Pfad jeder Figur eine eindeutige Farbe zugeordnet. Alle Pfeile der Form $(s_i,s_j, p)$ mit Gewicht $w$ sind dann in der zu $p$ zugehörigen Farbe gezeichnet, und die Dicke des Pfeilzugs ist linear in Abhängigkeit von $w$.}
...
Selbe Abbildung zeigt schon, wie solche Situationsgraphen ein graphischen Überblick über das Werk ermöglichen.
Die Veränderungen der Konfigurationen im Verlauf des Werks deutlich, ähnlich wie in Konfigurationsmatrizen.
Es sei hier anzumerken, dass sich eine Konfigurationsmatrix in einen ungewichteten Situationsgraph umrechnen lassen kann und umgekehrt.
Die beiden Darstellungsformen sind im Wesentlichen äquivalent.
...
Deshalb nehmen wir noch eine Vereinfachung des Situationsgraphen vor, und transformieren diesen in einen \emph{einfachen} gerichteten azyklischen Graphen.
Hierzu fassen wir alle parallelen Pfeile in einen einzigen zusammen, und summieren deren Gewichte.
Präziser:
\begin{em}%
...
\end{em}%
Abbildung \ref{fig:situationsgraph-einfach} zeigt den einfachen Situationsgraphen für \emph{Emilia Galotti}, hervorgegangen aus dem Situationsgraph $G$.
...
Nun können wir uns der ersten Behauptung zuwenden: Pfeile im einfachen Situationsgraph machen \textquote{kausale Zusammenhänge} zwischen Situationen deutlich.
Ein Pfeil $r$ von $s_i$ nach $s_j$ zeigt zeigt an, dass (mindestens) eine \emph{Handlung} in Situation $s_i$ kausal eine Handlung in Situation $s_j$ verursacht hat, bzw. ein Effekt dieser Handlung ist.
...
Damit erlaubt Beobachtung (i) einen restriktiven Charakter die notwendige (zunächst aber nicht hinreichende) Voraussetzung wird gesichert, dass $s_j$ auch tatsächlich \emph{in der Gesamthandlung} nach $s_i$ stattfindet.
Also dass Handlungen in überhaupt $s_i$ einen kausalen Effekt auf die Handlungen in $s_j$ haben kann.
...
Diese Informationsvergabe sollte einen Effekt auf die Informiertheit (oder Nicht-Informiertheit) der anderen Figuren haben.
...
Die Überlegungen scheinen zumindest für diese Instanz zuzustimmen.
Der Weg von II.3 zu V.5 scheint tatsächlich eine zumindest überwiegend plausible Handlungssequenz zu erklären:
...
Im vorigen Abschnitt wurde eine Korrespondenz zwischen Wegen im Situationsgraph und Handlungssequenzen im zugehörigen dramatischen Werk plausibilisiert.
Diese Einsicht greift dieser Abschnitt auf, und führt diese logisch fort.
...
Damit ist auch zu $G_\text{sim}$ eine gewichtsmaximale Wegüberdeckung $\mathcal{P}^*$ effizient berechenbar.
...
Eine solche gewichtsmaximale disjunkte Wegüberdeckung $\mathcal{P}^*$ kann eine sinnvolle Segmentierung der Gesamthandlung in Handlungssequenzen liefern.
...
\begin{enumerate*}[itemjoin=\\\hspace*{\parindent}] % hunspell: acc itemjoin
    ...
    \item Unter allen Wegüberdeckungen hat $\mathcal{P}^*$ das größte Gewicht.
        ...
        %Übertragen auf die Segmentierung bedeutet dies, dass eine solche Segmentierung gewählt wurde, welche die kausalen Verknüpfungen \emph{zwischen} den Sequenzen minimiert.
        Daraus ergibt sich, dass die entstandene Segmentierung tatsächlich \textquote{sinnvoll} im Sinne im Sinne unserer Zielsetzung ist, vgl. Abschnitt \ref{sec:handlung}:
...
\begin{SidewaysFigure}
    ...
    \caption{Einbettung des einfachen Situationsgraphen zu \emph{Götz von Berlichingen}.
        ...
        Zur besseren Sichtbarkeit ist die Dicke eines Pfeils $[r]$ proportional zu $w([r])^{2/3}$.
    Die gewichtsmaximale disjunkte Wegüberdeckung $\mathcal{P}^*$ ist einerseits durch rot hervorgehobene Pfeile visualisiert; rot hervorgehobene Ecken repräsentieren Wege der Länge 0. N.B.: Ecke bzw. Situation V.11 (\textquote{In einem Finstern engen Gewölbe}) ist isoliert, denn keine der in dieser Situation auftrennen Figuren tritt auch in einer anderen Situation auf.}
...
Das ist im ersten Teil die Handlung um Jagsthausen, die Festnahme Götzens, und deren Rückzug aus Bamberg auf seine Burg.
...
Diese Operationalisierung wurde anhand Goethes Dramas \emph{Götz von Berlichingen} vorgeführt. Im überwiegenden Maße ließ sich aus dem Ergebnis kausal bzw. thematisch abgetrennte Handlungssequenzen erkennen.
...
In diesem ist zu jedem Werk eine \textquote*{wahre} Plot-Strukturierung durch Expertinnen, Experten vorgegeben.
Nur durch Vergleich zwischen \textquote*{Wahrheit} und den Ergebnisse der Operationalisierung lassen sich allgemeine Schlüsse über die Mächtigkeit des hier vorgestellten Modell ziehen.\autocite[Vgl.][608]{jannidis_computergestuetzte_2017}
...
Eine Weiterentwicklung bzw. Korrektur des Verfahrens, immer gekoppelt am Plot-Konzept, kann auch Einsichten zum Begriff Plot liefern.\autocites[Vgl.][13]{moretti_operationalizing:_2013}[Vgl.][]{trilcke_small_2020}
...
Wir folgen hier Überlegungen von Trilcke und Fischer, und verstehen den Situationsgraphen an sich als das eigentliche \textquote{epistemische Ding} – nicht das repräsentierte Werk.
