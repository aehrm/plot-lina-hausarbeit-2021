...
\includepdf{./Deckblatt_Hausarbeit_Editionen_und_Korpora.pdf}
...
%
Nach Trilcke et~al. sei zentrale Schwachstelle dieser Figurennetzwerke, dass diese als \emph{statische} Modelle die zeitliche Dimension von Plot nicht abgebildet können.\autocite[Vgl.][176]{trickle_netzwerkdynamik_2017}
...
Aktuelle Entwicklungen in der Netzwerkanalyse versuchen daher, \emph{dynamische} Netzwerke zu entwickeln, um sie für die Plot-Analyse zu gebrauchen.\autocite[Vgl.][176]{trickle_netzwerkdynamik_2017}
...
Abschnitte \ref{sec:situationsgraph} und \ref{sec:segmentierung} wenden sich dem eigentlichen Ziel zu, und behandeln o.\,g. Punkte (i) und (ii): Erster plausibilisiert das Modell der \emph{Situationsgraphen}, Zweiter erklärt wie sich hieraus eine Plot-Struktur  errechnen lassen kann.
...
Es ist zwar klar, dass diese Annahmen im Allgemeinen nicht zutreffen, dieser Reduktionismus sei aber im Hinblick auf eine quantitative Auswertung großer digitaler TEI-genormter Korpora wie \emph{GerDraCor} ausgewählt.
Ein potentielles Modell muss mit diesen Imperfektionen der Eingangsdaten umgehen.
...
%Unmittelbar aus dieser Darstellungsweise folgt der Begriff des \emph{Wegs}:
Ein \emph{Weg} $P$ ist eine endliche Folge $(v_0, r_1, v_1, \dots, r_k, v_k)$, $k\geq 0$, sodass $v_0, \dots, v_k$ Ecken sind, $r_1, \dots, r_k$ Pfeile sind, sowie $\alpha(r_i) = v_{i-1}$, $\omega(r_i)=v_i$ für alle $i=1,\dots, k$.
...
Also in dem kein Weg $P$ der Länge $\geq 1$ existiert, sodass erste und letzte Ecke der Spur identisch sind.
...
Jede Zeile dieser Matrix repräsentiert hierbei eine Figur, jede Spalte eine Situation und dessen korrespondierende Konfiguration.
...
Mit dem Ziel, \textcquote[178]{trickle_netzwerkdynamik_2017}{Plot-Phänomene} zu quantifizieren, führt die dlina-Arbeitsgruppe \emph{segment-change rates} auf Basis der Konfigurationsmatrix ein.
Diese versucht, Handlungsentwicklung quantitativ zu erfassen.
%Sowohl die Arbeit der Arbeisgruppe, als auch die hier vorliegende Arbeit lässen sich der Konfigurationsanalyse zuordnen.
%
Die Arbeit sei hier deshalb diskutiert, weil sie – wie die hier vorliegende Arbeit – zur Konfigurationsanalyse gezählt werden kann; ferner haben beide Ansätze die Konfigurationsmatrix als Datengrundlage.\autocites{trickle_netzwerkdynamik_2017}{fischer_network_2017}{trilcke_small_2020}
...
Warum sie die \emph{segment-change rates} den dynamischen Netzwerkmaßen zuordnen, ist dem Autor der hier vorliegenden Arbeit nicht ersichtlich, sind ja die beschriebenen Konfigurationsmatrizen weder Netzwerke noch dynamisch.
...
\begin{em}%
    ...
    \begin{equation*} R = \{ (s_i,s_j,p) \mid i<j, p\in \mu(s_i), p \in\mu(s_j), \text{aber $p\not\in\mu(s_k)$ wenn immer $i<k<j$}  \}, \end{equation*}
    und für Pfeil $r=(s_i,s_j,p)\in R$ gilt
    %sei $\alpha(r) \coloneq s_i$, $\omega(r) \coloneq s_j$, $w(r) \coloneq \min\{ \rho(s_i, p), \rho(s_j, p) \}$.
    \begin{equation*} \alpha(r) = s_i, \quad\omega(r) = s_j, \quad w(r) = \min\{ \rho(s_i, p), \rho(s_j, p) \} .\end{equation*}
\end{em}
...
\begin{SidewaysFigure}
    ...
    \caption{Selbe Einbettung des einfachen Situationsgraphen zu \emph{Emilia Galotti}. Die rot hervorgehobenen Pfeilzüge entsprechen dem auf S.~\pageref{pos:weg-diskussion} diskutierten Weg.} 
...
%Pfeile im einfachen Situationsgraph machen \textquote{kausale Zusammenhänge} zwischen Situationen deutlich.
...
In dem hier vorliegenden Fall gilt $w(r) = \sum_{p\in P} \min \{ \rho(s_i, p), \rho(s_j, p) \}$.
Ein kausaler Effekt durch Figur $p$ wird also dann festgemacht, wenn diese in beiden Situationen viel spricht.
...
Deshalb betrachten wir im Folgenden \emph{gewichtsmaximale disjunkte Wegüberdeckungen} in $G_\text{sim}$.
Wir verwenden dabei folgende Definition:
\emph{Das Gewicht einer disjunkten Wegüberdeckung $\mathcal{P}$ eines mit $w$ gewichteten Graphen ist $w(\mathcal{P}) = \sum_{P\in\mathcal{P}} \sum_{r\in P} w(r)$.}
...
Es folgen nun Hinweise, dass diese Operationalisierung das in der Einleitung gewünschte Ziel erreicht.
...
\begin{enumerate*}[itemjoin=\\\hspace*{\parindent}] % hunspell: acc itemjoin
    ...
    \item Unter allen Wegüberdeckungen hat $\mathcal{P}^*$ das größte Gewicht.
        ...
        Daraus sollte sich ergeben, dass die entstandene Segmentierung tatsächlich \textquote{sinnvoll} im Sinne im Sinne unserer Zielsetzung ist, vgl. Abschnitt \ref{sec:handlung}:
...
\begin{SidewaysFigure}
    ...
    \caption{Einbettung des einfachen Situationsgraphen $G_\text{sim}$ zu \emph{Götz von Berlichingen}.
        ...
        Ein Pfeil $r$ ist rot hervorgehoben genau dann wenn $r\in P$ für einen Weg $P\in\mathcal{P}^*$.
        Eine Ecke $s$ ist rot hervorgehoben genau dann wenn $s\in P$ für einen Weg $P\in\mathcal{P}^*$ \emph{sowie} Länge von $P$ gleich 0.}
...
hier präsentiert sich \textquote{die Geschichte nicht mehr als geschlossenes, hierarchisches Ganzes}.\autocite[Vgl.][323]{pfister_drama:_2001}
...
Folgt man Moretti, so kann eine Weiterentwicklung bzw. Korrektur des Verfahrens – immer gekoppelt am Plot-Konzept – auch Einsichten zum Begriff \textquote{Plot} liefern.\autocites[Vgl.][13]{moretti_operationalizing:_2013}[Vgl.][]{trilcke_small_2020}
