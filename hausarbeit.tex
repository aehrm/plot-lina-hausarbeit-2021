...
\begin{enumerate*}
    ...
    \item Die Konfiguration einer Situation/Szene ergibt sich aus den Replikenmenge der Szene. % hunspell: acc Replikenmenge
        Wir nehmen also an, dass nur jene Figuren auf der Bühne in einer Szene präsent sind, welche auch sprechen.% – mehr Informationen zur Konfiguration liegen in \emph{GerDraCor} ohnehin nicht vor.
\end{enumerate*}
...
\emph{Das Gewicht einer disjunkten Wegüberdeckung $\mathcal{P}$ eines mit $w$ gewichteten Graphen ist $w(\mathcal{P}) = \sum_{P\in\mathcal{P}} \sum_{r\in R(P)} w(r)$. Also das gesamte Gewicht aller durclaufenen Pfeile aller Wege in $\mathcal{P}$}
